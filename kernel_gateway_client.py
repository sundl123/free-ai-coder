...
class GatewayClient:
    ...
    *** E X P E R I M E N T A L *** *** E X P E R I M E N T A L ***
    ...
    DEFAULT_USERNAME = os.getenv("KERNEL_USERNAME", "code-interpreter")
...
class KernelClient:
    ...
    def execute(self, code, timeout=REQUEST_TIMEOUT):
        ...
        try:
            ...
            while True:
                ...
                print(f'response_message_raw: {json.dumps(response_message)}')
                ...
                if response_message:
                    ...
                    if response_message_type == "error" or (
                        ...
                        response.append(
                            {
                                "msg_type": "stream",
                                "content": KernelClient._convert_raw_response(response_message["content"]["text"]),
                            }
                        )
                        ...
                        if "text/plain" in response_message["content"]["data"]:
                            response.append(
                                {
                                    "msg_type": "text/plain",
                                    "content": KernelClient._convert_raw_response(
                        ...
                        if "image/png" in response_message["content"]["data"]:
                            response.append(
                                {
                                    "msg_type": "image/png",
                                    "content": KernelClient._convert_raw_response(
                                        response_message["content"]["data"]["image/png"]
                                    )
                                }
                            )
    ...
    def execute_file(self, file_path, timeout=REQUEST_TIMEOUT):
        if not os.path.isfile(file_path):
            raise FileNotFoundError(f"File '{file_path}' not found")

        with open(file_path, "r") as file_handle:
            return self.execute(file_handle.read(), timeout=timeout)
